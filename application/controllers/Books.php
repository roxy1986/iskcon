	...
	public function __construct()
   {
	...
	public function sale($book_id = '') 
	{
		if($this->input->post('sale_book'))
		{
      	...
      	{
	      	$data['book_data'] = $this->Book_model->view_book($book_id);
				$this->load->view('books/sale', $data );
      	}
      	else
      	{
         	 $result = $this->Book_model->sale_book();
				 if($result == false)
				 {
						$this->session->set_flashdata('error_msg','Book sale not update, Please try again');
      		 }
      		 else 
      		 {
      		 	$this->session->set_flashdata('success_msg','Book Sales update successfully');
      		 }
      		 redirect('books/sale/'.$book_id);
      	}
		}
		else 
		{
